...
"""Framework-independent recorded initialization from a LeRobot v2.1 tree.

Shared by Isaac Lab's neural backend and RLinf's existing world-model environment.
It emits ``{"start_items", "target_items", "task", "episode_index", "dataset_meta"}``
and preserves the original frame alignment and key-frame initialization contract.
...
class LeRobotV21InitDataset(Dataset):
    ...
    @staticmethod
    def _table_column_as_array(table, column: str) -> np.ndarray | None:
...
class MixedLeRobotV21InitDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        mixing_weights: Sequence[float] | None = None,
        **dataset_kwargs,
    ) -> None:
        if not data_paths:
